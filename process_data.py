...
for path_year in os.listdir(path):
    ...
    for path_file in os.listdir(path_year):
        ...
        if os.path.isfile(path_file):
            ...
            day_value.append(df_file_csv['最新'].max())
            day_value.append(df_file_csv['最新'].min())
        max_min_value.append(day_value)
    print("已处理一年。。。")
df = pd.DataFrame(max_min_value,columns = ['日期','最高价','最低价'])

writer = pd.ExcelWriter('D:\\金融数据\\data\\process_data03-10\\03-10-price.xlsx')
